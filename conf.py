...
_C.PATCH.EPSILON = 1e-5
_C.PATCH.lamb_src = 0
_C.PATCH.GENT_PAR = 0.0
# --------------------------------- CUDNN options --------------------------- #
_C.CUDNN = CfgNode()
...
def load_cfg_from_args():
    ...
    if cfg.SETTING.DATASET == 'domainnet126':
        ...
        cfg.class_num = 126
        cfg.bottleneck = 256
    if cfg.SETTING.DATASET == 'oct':
        cfg.domain = ['Optovue', 'OCTID']
        cfg.class_num = 4 
        cfg.name_file = './data/oct/classname.txt'
    if cfg.SETTING.DATASET == 'fundus_4c':
        cfg.domain = ['ODIR', 'FIVES', 'VietAI']###
        cfg.class_num = 4 
        cfg.name_file = './data/fundus_4c/classname.txt'
    if cfg.SETTING.DATASET == 'fundus_dr':
        cfg.domain = ['SYSU', 'MESSIDOR']
        cfg.class_num = 5 
        cfg.name_file = './data/fundus_dr/classname.txt'
