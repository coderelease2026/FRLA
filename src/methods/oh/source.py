...
class ImageScaling():
    def __init__(self, size=(512, 512), canvas=True):
        self.size = size
        self.canvas = canvas
    ...
    def __call__(self, img):
        ...
        img = transforms.Resize((int(img.shape[-2]/scale), int((img.shape[-1]/scale))))(img)
...
def data_load(cfg): 
    ...
    if cfg.SOURCE.TRTE == "val":
        ...
        if 'fundus' in cfg.SETTING.DATASET: #handle data imbalance
            tr_label = [int(tr_txt[i].split(' ')[-1]) for i in range(len(tr_txt))]
            label_counts = Counter(tr_label)
            num_samples = len(tr_label)
            class_weights = {label: num_samples / count for label, count in label_counts.items()}
            sample_weights = [class_weights[label] for label in tr_label]
    ...
    dsets["source_tr"] = ImageList(tr_txt, transform=image_train(fundus=('fundus' in cfg.SETTING.DATASET)))
    
    if 'fundus' in cfg.SETTING.DATASET:
        dset_loaders["source_tr"] = DataLoader(dsets["source_tr"], batch_size=train_bs, num_workers=cfg.NUM_WORKERS, drop_last=False, sampler=sampler)
    else:
    ...
    dsets["source_te"] = ImageList(te_txt, transform=image_test(fundus=('fundus' in cfg.SETTING.DATASET)))
    ...
    dsets["test"] = ImageList(txt_test, transform=image_test(fundus=('fundus' in cfg.SETTING.DATASET)))
...
def cal_acc(loader, netF, netB, netC, flag=False):
    ...
    all_prob_max, predict = torch.max(all_output, 1)
    accuracy = torch.sum(torch.squeeze(predict).float() == all_label).item() / float(all_label.size()[0])
    mean_ent = torch.mean(loss.Entropy(all_output)).cpu().data.item()
   
    ###check confidence for correct/wrong predictions
    correct_confi_avg = (all_prob_max[predict==all_label]).mean()
    wrong_confi_avg = (all_prob_max[predict!=all_label]).mean()
    log_str = '{}, {}'.format(correct_confi_avg, wrong_confi_avg)
...
def train_source(cfg):
    ...
    while iter_num < max_iter:
        ...
        """ labels_count = [(labels_source == value).sum().item() for value in range(5)]
        print(labels_count) """
        iter_num += 1
        ...
        if iter_num % interval_iter == 0 or iter_num == max_iter:
            ...
            test_target(cfg, netF, netB, netC)
...
def test_target(cfg, netF=None, netB=None, netC=None):
    if netF is None:
        ## set base network
        if cfg.MODEL.ARCH[0:3] == 'res':
            netF = network.ResBase(res_name=cfg.MODEL.ARCH).cuda()
