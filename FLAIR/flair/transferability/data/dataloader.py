...
from FLAIR.flair.pretraining.data.dataset import Dataset
from FLAIR.flair.pretraining.data.transforms import LoadImage, ImageScaling, CopyDict
...
def get_dataloader_splits(dataframe_path, data_root_path, targets_dict, shots_train="80%", shots_val="0%",
                          ...
                          size=(512, 512), resize_canvas=False, batch_size_test=1,return_list=False):
    ...
    if return_list:
        test_list = [data["image_path"] for data in data_test]
        return loaders, test_list
    return loaders
