...
class VisionModel(torch.nn.Module):
    ...
    def forward(self, pixel_values, return_last_fea=False):
        if return_last_fea and (not self.hooked):

            def hook_function(module, input, output):
                self.last_features.append(output)
            self.model._modules.get("layer4").register_forward_hook(hook_function)
            self.hooked = True
        ...
        if return_last_fea:
            B,C,H,W = self.last_features[0].shape
            last_fea = self.last_features[0].permute(0,2,3,1)
            project_fea = self.projection_head_vision.projection(last_fea)
            #project_fea = project_fea.transpose(0,3,1,2)
            #normalize:
            project_fea_avg = torch.mean(project_fea, dim=(1,2), keepdim=True)
            project_fea = project_fea / project_fea_avg.norm(dim=-1, keepdim=True)

            #print((project_fea.mean(dim=(1,2))-embed).norm(dim=-1))
            #
            self.last_features.pop(0)
            return embed, project_fea #B,H,W,C
        return embed
