...
def process(args):
    ...
    for iFold in range(args.folds):
        ...
        # Set datasets
        args.loaders, test_list = get_dataloader_splits(args.setting["dataframe"], args.data_root_path, args.setting["targets"],
        ...
        if args.loaders["test"] is not None:
            refs, preds, patch_preds = adapter.predict(args.loaders["test"], return_cam=True)
...
"""     # Get metrics averaged across folds
    ...
    return test_list, refs, preds, patch_preds """

def main(dataset, domain):
    if dataset == 'fundus_4c':
        if domain == 1:
            dataset = '13_FIVES'
        elif domain == 2:
            dataset = '38_VietAI'
    elif dataset == 'fundus_dr':
        dataset = '02_MESSIDOR'
    else:
    ...
    # Experiment
    parser.add_argument('--experiment', default=dataset,
    ...
    parser.add_argument('--batch_size_test', default=64, type=int)#4
